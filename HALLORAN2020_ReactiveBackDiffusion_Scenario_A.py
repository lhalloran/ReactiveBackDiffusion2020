...
Python script to process and analyse model output for Scenario A in paper.
...
cutOffC = 0.00001    # Cut-off normalised concentration (>1E-5).
pointNumber = 5 - 1  # Observation well for analysis. Starts at 0, moving L to R.
##################################################################################################
...
fileOutName=fileName+'_OUT_'+dtNow+'.pdf'
...
#%% plots vs combined parameter chi
nperz=int(nParamsCombo/3) #assuming 3 depths

tDropMasked = np.ma.masked_where(tDrop==201,tDrop) # this is used for nDrop in order to mask the values that are > something
...
# pi1 = z*sqrt(Rc/De), pi2=K*sqrt(De)/(r^(3/2)). De=Df*epsilon^(2/3)
pi1=params[:,4]*np.sqrt(params[:,2]/(params[:,3]*epsilonAquitard**(2/3)))
pi2=params[:,0]*np.sqrt(params[:,3]*epsilonAquitard**(2/3))/params[:,2]**(3/2)

alpher=0.6
nvatt_masked = np.ma.masked_where(tDrop==201,nvatt)

#%% pi-theorem plot (tDrop vs Pi1):
fig = plt.figure(figsize=(6,6))
plt.loglog(pi1[0:nperz],tDropMasked[0:nperz,pointNumber],'go',pi1[nperz:nperz*2],tDropMasked[nperz:nperz*2,pointNumber],'b^',pi1[nperz*2:nperz*3],tDropMasked[nperz*2:nperz*3,pointNumber],'rv',alpha=alpher)
plt.legend(['1 m','0.2 m','5 m'])
plt.ylabel('tDrop [years]')
plt.xlabel('$\Pi_1$ [-]')
#
pp.savefig(fig)
#%% pi-theorem plot (tDrop vs Pi1):
fig = plt.figure(figsize=(6,6))
plt.loglog(pi2[0:nperz],tDropMasked[0:nperz,pointNumber],'go',pi2[nperz:nperz*2],tDropMasked[nperz:nperz*2,pointNumber],'b^',pi2[nperz*2:nperz*3],tDropMasked[nperz*2:nperz*3,pointNumber],'rv',alpha=alpher)
plt.legend(['1 m','0.2 m','5 m'])
plt.ylabel('tDrop [years]')
plt.xlabel('$\Pi_2$ [-]')
#
pp.savefig(fig)
#%% pi-theorem plot (nDrop vs Pi1) :
fig = plt.figure(figsize=(6,6))
plt.loglog(pi1[0:nperz],nDrop[0:nperz],'go',pi1[nperz:nperz*2],nDrop[nperz:nperz*2],'b^',pi1[nperz*2:nperz*3],nDrop[nperz*2:nperz*3],'rv',alpha=alpher)
plt.legend(['1 m','0.2 m','5 m'])
plt.ylabel('nDrop [-]')
plt.xlabel('$\Pi_1$ [-]')
#
pp.savefig(fig)
#%% pi-theorem plot (nDrop vs Pi2) :
fig = plt.figure(figsize=(6,6))
plt.loglog(pi2[0:nperz],nDrop[0:nperz],'go',pi2[nperz:nperz*2],nDrop[nperz:nperz*2],'b^',pi2[nperz*2:nperz*3],nDrop[nperz*2:nperz*3],'rv',alpha=alpher)
plt.legend(['1 m','0.2 m','5 m'])
plt.ylabel('nDrop [-]')
plt.xlabel('$\Pi_2$ [-]')
#
...
pp.savefig(fig)
#%% ETA PARAMETER
# eta = sqrt(r_1*D_e)/(K_a*z_a)
eta = np.sqrt(params[:,3]*params[:,2]*epsilonAquitard**(2/3))/(params[:,0]*params[:,4])
